...
- `asterisk-chan-dongle/` владеет всеми реализациями, доказанно оставшимися без
  изменений.
- В `src/` находятся 12 новых Svistok-файлов и только новые/изменённые единицы,
  извлечённые из 28 изменённых legacy-файлов, а также небольшие target-owned
  файлы интеграции ABI/state. Неизменённых тел функций в `src` нет.
...
baseline-срез и одну нефильтруемую composition, которая включает физический
delta-файл из `src` как есть. Неизменённые тела, например `app_register()`,
